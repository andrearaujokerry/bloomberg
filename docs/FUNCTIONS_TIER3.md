...
| Key-rate durations | tenors `2Y 5Y 10Y 30Y`; bump the **zero** curve of the bond's pricing curve by ±1 bp with a triangular kernel (peak at the key tenor, zero at the neighbouring key tenors, flat beyond the ends), reprice with the bond's z-spread held fixed, `KRD_t = −(P⁺ − P⁻) / (2 × P × 0.0001)`. The kernel and the shift are one implementation, `core/analytics/curve/keyRate.ts`, shared by YAS (`bond.krd@1.0.0`) and SWPM (`swap.ois` reprices), and the shift is applied in log-discount space (`df′(t) = df(t)·exp(−w(t)·δ·t)`, a rise in the **continuously compounded** zero) so that one basis point is the same perturbation whatever basis a curve quotes its own zeros on. The kernels are a partition of unity, so `Σ KRD` is the curve's **parallel-shift** duration to within the bump's truncation (~1e-5 relative). `Σ KRD ≈ modified duration` only up to that plus two structural gaps — the `y/2` between a continuous zero shift and a semiannual yield derivative, and the curve's slope over the bond's life — so it is **asserted in tests within 3 %**, not 2 %: the measured residuals on the seeded `UST_PAR` curve are +1.88 % (2Y), +1.91 % (10Y), +1.93 % (off-the-run 2034) and −0.04 % (30Y), and a 2 % bound would pass only because every seeded yield happens to sit below 4.1 %. A key-rate duration that summed to the modified duration *exactly* would be an algebraic slice of it (`bond/risk.ts#keyRateDurations`, which takes a yield, not a curve) and would carry no twist information at all — the defect WP-11 shipped in YAS |
...
export type YasPayload = {
  ...
  results:
    | { kind: 'coupon';
        ...
        keyRateDurations: Array<{ tenor: '2Y'|'5Y'|'10Y'|'30Y'; krd: number | null }>;   // null = no pricing curve or no solved z-spread (§0)
...
| Engines (`core/analytics`) | `bond/cashflows@1.0.0`, `bond/price@1.0.0`, `bond/risk@1.0.0`, `bill@1.0.0`, `curve/bootstrap@1.0.0`, `curve/interp@1.0.0`, plus `bond.zspread@1.0.0` and `bond.krd@1.0.0` defined in `manifests/YAS.ts` (no curve-aware entry point exists in `bond/risk.ts`) |
...
| KRD sum | `packages/core/test/analytics/curve/keyRate.test.ts` · `packages/server/test/integration/functions/YAS.test.ts` | core: the §0 kernels are a partition of unity and `Σ KRD` reproduces a single parallel 1 bp bump within 1e-4 relative (and is not equal to it); the 30y bucket of a note maturing in 2036 is non-zero, which a yield-space tent cannot produce. Integration: `Σ KRD` within **3 %** of modified duration for every seed note/bond and never exactly equal to it (§0); `KRD_*` are `null` with a `NO_SOURCE` note when no curve is stored; z-spread of a bond priced off the curve is 0 ± 0.01 bp |
