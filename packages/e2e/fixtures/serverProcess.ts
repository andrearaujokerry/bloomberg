// WP-01 scaffold — ARCHITECTURE §3.5 L398-400.
//
// One place that knows how to start the plant in replay mode against `bloomberg_test`, and how to
// wait for it to be healthy. `playwright.config.ts` uses the constants for its `webServer` block;
// specs that need a plant of their own (a restart, a frozen feed) use `startServer()`.
//
...
export const SERVER_PORT = Number(processEnv.TERMINAL_E2E_SERVER_PORT ?? '8080');
export const WEB_PORT = Number(processEnv.TERMINAL_E2E_WEB_PORT ?? '5173');
...
export const TEST_DATABASE_URL =
  processEnv.DATABASE_URL_TEST ?? 'postgres://localhost:5432/bloomberg_test';
 ...
 * the test database, never `bloomberg_dev`.
 */
